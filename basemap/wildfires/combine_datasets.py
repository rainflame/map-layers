...
def cli(bbox, usfs_input_file, blm_input_file, nifc_input_file, output_file):
    ...
    def process_nifc_feature(feature):
        ...
        if feature["properties"].get('year', None) == None or feature["properties"].get('year', None) == "":
            return
    ...
    print(f"Found {len(cleaned_usfs_features)} USFS features, {len(cleaned_blm_features)} BLM features, and {len(cleaned_nifc_features)} NIFC features")

    # save the features to a geopackage
    print("Saving features to geopackage...")

    output_features = cleaned_usfs_features + cleaned_blm_features + cleaned_nifc_features
    schema = {
    ...
    # Open the GeoPackage file in write mode
    with fiona.open(output_file, 'w', driver='GPKG', schema=schema, crs=from_epsg(4326)) as output_geopackage:
